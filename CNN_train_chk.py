...
config = tf.ConfigProto( device_count = {'GPU': 1 , 'CPU': 2} , log_device_placement = True)
...
files = os.listdir('/home/parthasarathidas/Documents/train_set2/')
...
def data(filename):
    ...
    for f in filename:
    	if f.endswith(".jpg") or f.endswith(".PNG") or f.endswith(".TIFF"):
           current = f
           org_img =Image.open('/home/parthasarathidas/Documents/train_set2/'+current)
...
x = np.reshape(x, (np.size(files), 224, 224, 3)) #for image of pixels 100x100 only
...
#y_train = np_utils.img_to_arraycategorical(y_train, num_classes = 7)
...
model = Sequential()  
model.add(Conv2D(16, (4, 4), activation = 'relu', input_shape = x.shape[1:]))
...
if os.path.isfile(os.path.join(os.getcwd()+"save-best-weights.hdf5")) == True:
	model.load_weights("save-best-weights.hdf5")
	model.compile(optimizer= ada, loss = 'binary_crossentropy',
              metrics= ['accuracy'])
	history = model.fit(x, y, batch_size= 16, epochs= n_iter, validation_split=0.2, callbacks = callbacks_list, verbose = 0)
else:
	model.compile(optimizer= ada, loss = 'binary_crossentropy',
              metrics= ['accuracy'])
	filepath="save-best-weights.hdf5"
