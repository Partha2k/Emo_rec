...
for i in range(0, len(filenames)):
    ...
    current = filenames[i]  
    if current != '.DS_Store':
        image = np.array((Image.open('/home/parthasarathidas/Documents/results/' + current)).getdata())
        image = np.reshape(image, (64, 64))
        for i in range(0, 16):
            if files == 'train_files.txt':
                imsave('/home/parthasarathidas/Documents/results/aug_data_64_by_48/' + str(i) + '_' + current, image[i:i+48, i:i+48])  
            	#imsave('aug_train/' + str(i) + '_' + current, image[i:i+48, i:i+48])  

            else:
                #imsave('aug_test/' + str(i) + '_' + current, image[i:i+48, i:i+48])

	        imsave('/home/parthasarathidas/Documents/results/aug_test_data_64_by_48/' + str(i) + '_' + current, image[i:i+48, i:i+48])     
